...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        #check if orientation or encoder measurements are valid
        if self.orientation_val == True or self.encoder_val == True:
            hm = self.hm(xk)
        else:
            hm = np.zeros((0,1))
        #check if feature observations are valid
        if self.feature_val == True:
            map_i = []
            for i in range(len(self.H)):
                if self.H[i] is not None:
                    map_i.append(self.H[i])
            hf = self.hf(xk, map_i)
        else:
            hf = np.zeros((0,1))
        #stack the expected measurements and feature observations into a single vector
        h_mf = np.block([[hm], [hf]])
        return h_mf
    ...
    def ICNN(self, hf, Phf, zf, Rf, dim):
        ...
        H = []
        if zf is not None:
            #iterate over each observed feature and its covariance
            for zfi, Rfi in zip(zf, Rf):
                nearest = None
                D2_min = np.inf
                #iterate over each expected feature
                for i in range(self.nf):
                    #calculate the Mahalanobis distance between the observed and expected feature
                    D2_ij = self.SquaredMahalanobisDistance(hf[i], Phf[i], zfi, Rfi)
                    #check if the observed feature is individually compatible and has the smallest distance
                    if self.IndividualCompatibility(D2_ij, dim, self.alpha) and (D2_ij < D2_min):
                        nearest = i
                        D2_min = D2_ij
                #append the index of the nearest feature to the hypothesis list
                H.append(nearest)
        return H
    ...
    def DataAssociation(self, xk, Pk, zf, Rf):
        ...
        PF_list = []
        for i in range(self.nf):
            hF_list.append(self.hfj(xk,i)) #compute the expected observation of the i-th feature based on the current state xk
            j = self.Jhfjx(xk,i) #compute the Jacobian of the i-th feature observation 
            PFi =  j @ Pk @ j.T #compute the covariance of the i-th feature observation
            PF_list.append(PFi)  #append the computed covariance to the list
        self.H = self.ICNN(hF_list,PF_list,zf,Rf,self.zfi_dim)
        return self.H
    ...
    def Localize(self, xk_1, Pk_1):
        ...
        #self.PlotFeatureObservationUncertainty(zf_plot,  r_plot, 'y')
        ...
        #print(zf)
        #print(Rf)
        #self.PlotFeatureObservationUncertainty(zf,  Rf, 'y')
