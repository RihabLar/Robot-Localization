...
from MapFeature  import Cartesian2DStoredPolarObservedMapFeature  
import scipy
...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[],*args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        self.Polar_feature_reading_frequency = 1  # frequency of Polar2D feature readings
        self.Polar_max_range = 50  # maximum Polar2D range, used to simulate the field of view
        self.Rfp = np.diag(np.array([1 ** 2, np.deg2rad(5) ** 2]))  # covariance of simulated Polar2D feature noise
        
        self.Cartesian_feature_reading_frequency = 1  # Frequency of Cartesian2D feature readings
        self.Cartesian_max_range = 50  # Maximum Cartesian2D range, used to simulate the field of view
        self.Rfc = np.diag(np.array([0.5 ** 2, 1 ** 2]))   # Covariance of simulated Cartesian2D feature noise
    ...
    def ReadCartesianFeatureObservation(self):
        #store observed features in cartesian coordinates
        zsk = []
        Rsk = []
        xsk = Pose3D(self.xsk[0:3, 0].reshape((3, 1)))
        # Loop through each feature in the map
        for i in range(self.nf):
            noise = np.random.normal(0, np.sqrt(self.Rfc.diagonal())).reshape((len(self.Rfc), 1))
            feature_position = self.M[i].boxplus(xsk.ominus()) + noise
            distance = np.linalg.norm(feature_position[:2])  # Calculate distance in 2D

            if distance <= self.Cartesian_max_range:
                zsk.append(feature_position)
                Rsk.append(self.Rfc)
        # return the observed features and their covariances at the specified reading frequency
        if self.Cartesian_feature_reading_frequency != 0 and self.k % self.Cartesian_feature_reading_frequency == 0:
            return zsk, Rsk
        else:
            return np.zeros((0, 1)), np.zeros((0, 0))

        
    def ReadPolarFeatureObservation(self):
        #store observed features in polar coordinates
        zsk = []
        Rsk = []

        for i in range(self.nf):
            noise = np.random.normal(0, np.sqrt(self.Rfp.diagonal())).reshape((len(self.Rfp), 1))
            feature_position = Cartesian2DStoredPolarObservedMapFeature().s2o(
                self.M[i].boxplus(Pose3D.ominus(self.xsk[0:3, 0].reshape((3, 1)))) + noise)
            # Calculate the distance from the robot to the feature in 2D
            distance = np.linalg.norm(feature_position[:2])

            if distance <= self.Polar_max_range:
                zsk.append(feature_position)
                Rsk.append(self.Rfp)

        if self.Polar_feature_reading_frequency != 0 and self.k % self.Polar_feature_reading_frequency == 0:
            return zsk, Rsk
        else:
            return np.zeros((0, 1)), np.zeros((0, 0))
    
        
    def ReadPolarPolarFeatureObservation(self):
        #store observed features in polar coordinates
        zsk = []
        Rsk = []

        for i in range(self.nf):
            # Generate noise based on the covariance matrix
            noise = np.random.normal(0, np.sqrt(self.Rfp.diagonal())).reshape((len(self.Rfp), 1))
            feature_position = self.M[i] + noise
            # Calculate the distance from the robot to the feature in 2D
            distance = feature_position[0]  
            if distance <= self.Polar_max_range:
                zsk.append(feature_position)
                Rsk.append(self.Rfp)
