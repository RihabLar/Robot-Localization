...
class Cartesian2DMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        ...
        """
        # TODO: To be implemented by the student
        zk, Rk = self.robot.ReadCartesianFeatureObservation()
        if len(zk) != 0:
            self.feature_val = True
        else: 
            self.feature_val = False
        return zk, Rk
...
class Cartesian2DStoredPolarObservedMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        ...
        zk, Rk = self.robot.ReadPolarFeatureObservation()
        if len(zk) != 0:
            self.feature_val = True
        return zk, Rk
    ...
    def s2o(self, v):
        ...
        v = CartesianFeature(np.block([np.sqrt(v[0]**2+v[1]**2), np.arctan2(v[1], v[0])]).reshape(2,1))
        return v
    
    def o2s(self, v):
        
        v = CartesianFeature(np.block([v[0]*np.cos(v[1]),v[0]*np.sin(v[1])]).reshape(2,1))
...
class PolarMapFeature(MapFeature):
    ...
    def GetFeatures(self):
        zk, Rk = self.robot.ReadPolarPolarFeatureObservation()
