...
class CartesianFeature(Feature,np.ndarray):
    ...
    def boxplus(self, NxB):
        ...
        NxF = self.F @ NxB.oplus((self.F).T @ self.feature)
        NxF= CartesianFeature(NxF)
        return NxF

        
    def J_1boxplus( self, NxB):
        """
        ...
        # TODO: To be completed by the student
        J = self.F @ NxB.J_1oplus((self.F).T @ self.feature)
        return J
    ...
    def J_2boxplus(self,NxB):
        ...
        # TODO: To be completed by the student
        J = self.F @ NxB.J_2oplus() @ (self.F).T
        return J
...
class PolarFeature(Feature, np.ndarray):
    ...
    def __new__(BxF, input_array):
        # Ensure input_array is of the correct shape
        assert input_array.shape == (3, 1) or input_array.shape == (2, 1), "PolarFeature must be of 2 or 3 DOF"

        obj = np.asarray(input_array).view(BxF)

        BxF.feature = obj

        # Initialize the parent classes (Feature and np.ndarray)
        super().__init__(BxF, obj)
...
if __name__ == '__main__':
    NxB3dof = Pose3D(np.array([[5, 5, np.pi/2]]).T)
    BxF_polar = PolarFeature(np.array([[3, np.pi/4]]).T)

    NxF = BxF_polar.boxplus(NxB3dof)

    print("NxF (Polar) =", NxF.T)
    print("J_1boxplus (Polar) =", BxF_polar.J_1boxplus(NxB3dof))
    print("J_2boxplus (Polar) =", BxF_polar.J_2boxplus(NxB3dof))
